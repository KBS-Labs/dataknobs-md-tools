...
class TestMakeSvgIdsUnique:
    ...
    def test_no_double_suffix(self):
        ...
        """
        svg = (
            '<svg id="my-svg"><style>#my-svg { fill: red; } #my-svg-1 { fill: blue; }</style></svg>'
        )
        result = make_svg_ids_unique(svg, "3")
...
class TestNormalizeSvgSizing:
    ...
    def test_removes_large_max_width(self):
        ...
        svg = (
            '<svg viewBox="0 0 3467 996"'
            ' style="max-width: 3467.26px; background-color: white;"></svg>'
        )
    ...
    def test_preserves_other_attributes(self):
        ...
        svg = (
            '<svg viewBox="0 0 1000 500" class="flowchart"'
            ' id="my-svg" style="max-width: 1000px;"></svg>'
        )
        result = normalize_svg_sizing(svg)
